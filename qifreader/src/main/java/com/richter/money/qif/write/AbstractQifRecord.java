...
public abstract class AbstractQifRecord implements QifRecord {
	...
	private void appendStringValueToOutput(String key, String value,
			StringBuilder sb) {
	...
	private void appendDateValueToOutput(String key, LocalDate date,
			StringBuilder sb) {
		if (date != null) {
			if (sb.length() > 0) {
				sb.append(LINE_SEPARATOR);
	...
	private void appendBigDecimalValueToOutput(String key, BigDecimal value,
			StringBuilder sb) {
		if (value != null) {
			if (sb.length() > 0) {
				sb.append(LINE_SEPARATOR);
	...
	protected void appendFieldValueToOutput(String key, Object value,
			StringBuilder sb) {
